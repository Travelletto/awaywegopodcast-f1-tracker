...
function createUser(username, token, email, emailOptin) {
  const stmt = db.prepare(
    'INSERT INTO users (username, token, email, email_optin) VALUES (?, ?, ?, ?)'
  );
  return stmt.run(username, token, email || null, emailOptin ? 1 : 0);
}
...
function updateUserEmail(userId, email, emailOptin) {
  return db.prepare('UPDATE users SET email = ?, email_optin = ? WHERE id = ?')
    .run(email || null, emailOptin ? 1 : 0, userId);
}
